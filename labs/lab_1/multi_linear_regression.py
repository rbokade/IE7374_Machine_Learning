...
class MultiLinearRegression(BaseLearner):
    ...
    def print_performance(self, X, y):
        ...
        print("Intercept: {}".format(self._thetas[0]))
        print("Coefficients: {}".format(" ".join(str(theta) for theta in self._thetas[1:]), sep=" ", end=" "))
...
if __name__ == "__main__":
    ...
    print("coeffs", mlr.fit(X, y))
    norm_test, _, _ = loader.normalize_data(test, means, stds)
    print("test performance", mlr.evaluate(norm_test.iloc[:, :-1], norm_test.iloc[:, -1]))
